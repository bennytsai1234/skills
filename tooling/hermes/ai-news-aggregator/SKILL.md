...
```bash
# 抓取所有源（最近3天新闻）
python3 ~/skills/tooling/hermes/ai-news-aggregator/rss_aggregator.py --category all --days 3 --limit 10

# 只看公司博客
python3 ~/skills/tooling/hermes/ai-news-aggregator/rss_aggregator.py --category company --days 1 --limit 5

# 只看中文媒体
python3 ~/skills/tooling/hermes/ai-news-aggregator/rss_aggregator.py --category cn_media --days 3 --limit 10

# AI Agent 相关
python3 ~/skills/tooling/hermes/ai-news-aggregator/rss_aggregator.py --category ai-agent --days 7 --limit 10

# 输出 JSON 格式
python3 ~/skills/tooling/hermes/ai-news-aggregator/rss_aggregator.py --category all --days 1 --json
```

### arXiv 论文
```bash
# 最新 AI 论文（按热度排序）
python3 ~/skills/tooling/hermes/ai-news-aggregator/arxiv_papers.py --limit 5 --top 10

# 搜索特定主题
python3 ~/skills/tooling/hermes/ai-news-aggregator/arxiv_papers.py --query "multi-agent" --top 5
```

### GitHub Trending
```bash
# AI 相关热门项目（今日）
python3 ~/skills/tooling/hermes/ai-news-aggregator/github_trending.py --ai-only

# 本周热门
python3 ~/skills/tooling/hermes/ai-news-aggregator/github_trending.py --since weekly
```
...
编辑 `rss_sources.json` 添加/删除 RSS 源：
```json
{
  "name": "OpenAI Blog",
