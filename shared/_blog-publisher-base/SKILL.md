...
```
接下來依序套用 ~/skills/shared/_blog-publisher-base/SKILL.md 的 Pipeline 章節（從「Pipeline Step A」到「Pipeline Step F」），並使用下列 variant 變數：
...
| 檢查 | 通過標準 |
|------|----------|
| L2-1 開頭 | 第一句是具體事件 / 時間 / 產品，不是宏大敘事 |
| L2-2 段落節奏 | 長短句交替，不是連續 3 句以上相同句式 |
| L2-3 有觀點 | 每個主題段落都有「為什麼重要 / 誰受影響」的分析，不是純事實堆疊 |
...
> **「我把這段貼給一個不看 AI 新聞的工程師朋友，他會覺得這是值得花 3 分鐘讀的東西，還是又一篇 AI 生成的摘要？」**

如果答案是後者，找出最像 AI 摘要的兩段，重寫。
