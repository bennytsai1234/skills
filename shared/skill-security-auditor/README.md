...
chmod +x ~/skills/shared/skill-security-auditor/analyze-skill.sh

# Test it works
~/skills/shared/skill-security-auditor/analyze-skill.sh --help
```
...
mkdir -p ~/skills/shared/skill-security-auditor/patterns

# Download SKILL.md
curl -o ~/skills/shared/skill-security-auditor/SKILL.md \
  https://raw.githubusercontent.com/YOUR-USERNAME/skill-security-auditor/main/SKILL.md

# Download analyzer script
curl -o ~/skills/shared/skill-security-auditor/analyze-skill.sh \
  https://raw.githubusercontent.com/YOUR-USERNAME/skill-security-auditor/main/analyze-skill.sh

# Download patterns database
curl -o ~/skills/shared/skill-security-auditor/patterns/malicious-patterns.json \
...
~/skills/shared/skill-security-auditor/analyze-skill.sh -s solana-wallet-tracker
...
for skill in ~/.openclaw/skills/*/SKILL.md; do
  ...
  ~/skills/shared/skill-security-auditor/analyze-skill.sh -f "$skill"
  echo ""
done
...
echo 'alias audit-skill="~/skills/shared/skill-security-auditor/analyze-skill.sh"' >> ~/.bashrc
source ~/.bashrc
...
}]' ~/skills/shared/skill-security-auditor/patterns/malicious-patterns.json > temp.json
mv temp.json ~/skills/shared/skill-security-auditor/patterns/malicious-patterns.json
```
...
curl -s "https://openclaw-security.github.io/threat-intel/latest.json" \
  -o ~/skills/shared/skill-security-auditor/patterns/ioc-database.json
```
...
1. **Run pattern analysis** (local, instant):
   ```bash
   ~/skills/shared/skill-security-auditor/analyze-skill.sh -s suspicious-skill
   ```
