...
3. 每個主題預設只補 `1` 個原始來源（官方公告 / 原論文 / 官方文件）+ `1` 個側面來源
4. 把同一事件的多個來源合併成一個主題
5. 對至少一個關鍵事實做交叉比對（價格、日期、模型名、benchmark、融資額）
6. **遇到模型 / 產品新聞時，先查核心發布時間。** 若核心發布時間明顯早於當日，除非今天有重大新版、正式商用、價格變更、開源、API 開放或關鍵 benchmark 變化，否則降權到「其他值得關注」或直接跳過，不要硬寫成主題。

---

## Step 5｜寫作

繁體中文。套用 base 風格規則。預設固定寫 `3` 個主題，目標字數預設 **1100–1500 字**。結構：
...
簡繁殘留檢查請直接用 shell 跑，不要用代理式 `search`：

```bash
rg -n '視頻|软件|硬盤|运营|視頻版|并发|用户|会话|项目' "$POST_PATH"
```

若回傳 exit code 1，代表零命中，屬於正常通過。
