...
description: 每天台北時間 08:00 執行。從本地 RSS 彙整檔挑 3 個當天最重要的 AI 主題，寫成有深度觀點的「AI 晨間精選」。繼承 _blog-publisher-base。
---
...
**發文時段**：每天 `08:00`（台北）
**系列**：`daily-ai-report`
**定位**：精選、有深度、有觀點。從上百條 RSS 挑 3 個最值得花時間的主題，每個主題寫出分析段落。整篇目標字數預設 **1100–1500 字**。
...
同一事件有多條來源時合併寫一個主題。預設固定取 `3` 個主題，不主動擴到第 4、5 個。

---

## Step 3｜深度搜尋（不可偷懶）

RSS 摘要不夠時用 `web_search` / `web_fetch` 補，而且每個主題至少要補到以下兩類來源中的兩個：
...
每個主題預設上限是 `1` 個原始來源 + `1` 個側面來源。只有數字、時間、產品名稱互相打架時，才補第 3 個來源。
...
一段 4–6 句的總結性開場，把今天幾個主題的共同脈絡或產業趨勢串起來。
（記得不要寫「隨著 AI 的快速發展」這類空話開頭，要從具體事件切入。）

這段不只要總結，還要給讀者一個明確判斷：今天最大的變化是什麼。
