...
chmod +x ~/skills/skill-security-auditor/analyze-skill.sh

# Test it works
~/skills/skill-security-auditor/analyze-skill.sh --help
```
...
~/skills/skill-security-auditor/analyze-skill.sh -s solana-wallet-tracker
...
for skill in ~/.openclaw/skills/*/SKILL.md; do
  ...
  ~/skills/skill-security-auditor/analyze-skill.sh -f "$skill"
  echo ""
done
...
echo 'alias audit-skill="~/skills/skill-security-auditor/analyze-skill.sh"' >> ~/.bashrc
source ~/.bashrc
...
}]' ~/skills/skill-security-auditor/patterns/malicious-patterns.json > temp.json
mv temp.json ~/skills/skill-security-auditor/patterns/malicious-patterns.json
```
...
curl -s "https://openclaw-security.github.io/threat-intel/latest.json" \
  -o ~/skills/skill-security-auditor/patterns/ioc-database.json
```
...
1. **Run pattern analysis** (local, instant):
   ```bash
   ~/skills/skill-security-auditor/analyze-skill.sh -s suspicious-skill
   ```
