...
## 0. Token Efficiency Mode（預設開啟）

所有自動 blog skill 預設都要以「夠好就停」為原則，在不明顯傷害可讀性的前提下，優先壓低每日 cron token 消耗。

### 硬性節流規則

- 主題數預設取下限。若 variant 寫「3–5 個主題」，預設先取 `3` 個。
- 「其他值得關注」預設最多 `2` 則。
- 每個主題預設只補 `1` 個原始來源 + `1` 個側面來源。只有關鍵事實互相衝突時，才補第 3 個來源。
- 每篇文章至少交叉比對 `1` 個最關鍵的事實即可，不要求每個主題都交叉比對。
- 每個主題預設 `2` 段內講完；只有真的需要背景鋪陳時才寫第 3 段。
- 生成完正文後直接寫檔，不要再把整篇文章讀回上下文，除非 build 失敗或只想精修局部段落。
- cron log 的自檢輸出一律用精簡版，不要把大段分析或全文回貼到執行摘要。
...
- **短文下限**：700 字。除非走 Light Issue，否則不得低於此數。
- **標準技術解析**：900–1400 字。
- **深度研究 / 專題解析**：1400–2000 字。
- **每個主題段** 至少回答三件事：
  1. 它到底做了什麼
  2. 它跟既有做法差在哪裡
  3. 為什麼工程師現在要在意
...
| L3-5 研究完整度 | 至少 1 個原始來源 + 1 個側面來源；關鍵事實有交叉比對 |
...
variant 在 Pipeline Step D 結束後，只把自檢結果輸出到 STDOUT（這樣會進入 cron log），不要 append 到 POST_PATH 檔案裡。預設用精簡版，失敗時才補充問題段落。
...
- L1/L2 的文字掃描一律用 shell 指令完成，優先用 `rg`，沒有 `rg` 才退回 `grep -nE`。
- **不要**使用 `search ... in <file>` 這類代理工具語法做檔案掃描；cron 環境下這種做法可能直接失敗，會讓整篇文章明明寫好了卻卡死在自檢。
- 建議直接跑這類指令：

```bash
rg -n '說白了|本質上|綜上所述|總的來說|不可否認|值得注意的是|不難發現|這意味著|意味著什麼|換句話說|首先|其次|最後|讓我們來看看|接下來讓我們|在當今|隨著.+發展|AI 工具|某個模型|主流大廠|相關技術|視頻|軟件|硬盤|運營|^# ' "$POST_PATH"
rg -n '^## 參考連結' "$POST_PATH"
```

若 `rg` 對第一條回傳 exit code 1，代表「零命中」，這是通過，不是失敗。
